  ...
  ALIASES: get_output FOR lif_output~get_output,
    ...
    METHODS find_passenger_flight
      IMPORTING
        ...
        i_seats           TYPE i
      EXPORTING
        e_flight          TYPE REF TO lcl_flight    "Übung 20 Up Cast -> Übung 20 Up Cast
        e_days_later      TYPE i.
    ...
    METHODS find_cargo_flight
      ...
      EXPORTING
        e_flight          TYPE REF TO lcl_flight "Übung 20 Up Cast -> cargo_flight
        e_days_later      TYPE i.
...
* DATA cargo_flights TYPE lcl_cargo_flight=>tt_flights.
    ...
    data pf_count type i.                   "Übung 20.   Collection für alle Flüge
    data cf_count type i.                   "Übung 20.   Collection für alle Flüge
...
*    name = carrier_id && ` ` && name.    "Delete Übung 11
   ...
   loop at passenger_flights into data(passflight).
    append passflight to flights.                        "Übung 20. Flüge in diie Collection - Up cast
   endLOOP.

loop at cargo_flights into data(cargoflight).
    append passflight to flights.                        "Übung 20. Flüge in diie Collection - Up cast
   endLOOP.
*    me->passenger_flights = passenger_flights.
*   me->cargo_flights     = cargo_flights.
...
*Übung 14
    ...
    r_result = REDUCE #(  INIT i = 0
                           FOR <line> IN flights where ( table_LINE is instANCE OF lcl_passenger_flight )   "Übung20. Filter wegen
                           NEXT i = i + cast lcl_passenger_flight( <line> )->get_free_seats( )               "Übung20. Downcast
