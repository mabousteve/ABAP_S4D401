...
"! Abstrakte Oberklasse für die Klassen <strong>lcl_passenger_flight</strong> und <strong>lcl_cargo_flight</strong></br>
"! {@link .LCL_PASSENGER_FLIGHT}<br>
"! {@link .LCL_CARGO_FLIGHT}<br>
"! <br>Jede Instanz wird durch die Attribute <strong>carrier_id</strong>, <strong>connection_id</strong> und <strong>flight_date</strong> eindeutig identifiziert.</br>
...
"!Flight carrier
"! "! Eine Fariklogik stellt sicher, dass es nur eine Instanz für dieselbe Spediteur-ID gibt.
...
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @parameter r_result | Rreferenz auf die Instanz
    "! @raising zcx_06_failed | Aufnahmetext bei fehlgeschlagener Instanziierung
    ...
    "! @parameter i_seats | Mindestanzahl verfügbarer Plätze
    "! @parameter e_flight | gefundener Flug (Objektreferenz)
...
*    DATA name TYPE string.
    ...
    AUTHORITY-CHECK OBJECT '/LRN/CARR'
                 ...
                 ID 'ACTVT' FIELD '03'.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_06_failed
        EXPORTING
          textid     = zcx_06_failed=>carrier_no_read_auth
          carrier_id = i_carrier_id.
      .
    ENDIF.
    "Übung 18 <-
...
*     WHERE carrier_id = 'AAA'
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_06_failed
        EXPORTING
          textid     = zcx_06_failed=>carrier_not_exist
          carrier_id = i_carrier_id.
