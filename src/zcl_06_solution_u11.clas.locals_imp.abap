...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    LOOP AT connection_buffer INTO DATA(connection).
      "Lokale Zeit umgerechnet auf UTC Zeit -> Frankfurt 11:00 Uhr   --> UTC: 10:00 Uhr
      CONVERT DATE today TIME connection-departure_time
        TIME ZONE connection-timzone_from   "wegen Übung 10
*        TIME ZONE airports[ airport_id = connection-airport_from_id ]-timzone
            INTO UTCLONG DATA(departure_utclong).

      "Lokale Zeit umgerechnet auf UTC Zeit -> New York 12:00 Uhr     --> UTC: 17:00 Uhr
      CONVERT DATE today TIME connection-arrival_time
        TIME ZONE connection-timzone_to "wegen Übung 10
...
*      INTO TABLE @DATA(keys).
  ...
  METHOD constructor.
    ...
    IF flight_raw IS NOT INITIAL.
      ...
      "seats_free = flight_raw-seats_max - flight_raw-seats_occupied. "da Übung 11 -> diese Zeile auskommentieren wegen next zeile
      seats_free = flight_raw-seats_free.  "Übung 11 -> vorherige Zeile auskommentieren
...
CLASS lcl_carrier DEFINITION .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_output TYPE string. " Übung 06 - vorher: c LENGTH 25.
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    fieLDS CONCAT_WITH_SPACE( carrier_id, name, 1 ), currency_code "Übung 11 ohme As name
     WHERE carrier_id = @i_carrier_id
